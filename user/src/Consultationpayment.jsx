...
// ── Success Modal ─────────────────────────────────────────────────────────────
function SuccessModal({ sp, date, slot, onClose }) {
  return (
    <div className="cp-modal-overlay" onClick={onClose}>
      <div className="cp-modal" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:800,color:"#1a3329",marginBottom:8 }}>Booking Confirmed!</div>
        <div style={{ fontSize:14,color:"#5a7a6e",lineHeight:1.75,marginBottom:20 }}>
          Your consultation with <strong style={{color:"#1a3329"}}>{sp.name}</strong> has been successfully booked.
        </div>
        <div style={{ background:"#f7faf8",borderRadius:14,padding:"14px 16px",marginBottom:20,textAlign:"left" }}>
          <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:12 }}>
            <img src={sp.avatar} alt="" style={{ width:42,height:42,borderRadius:"50%",objectFit:"cover",flexShrink:0 }}/>
            <div>
              <div style={{ fontSize:13.5,fontWeight:700,color:"#1a3329" }}>{sp.name}</div>
              <div style={{ fontSize:12,color:"#4f9e7a" }}>{sp.title}</div>
            </div>
          </div>
          <div style={{ height:1,background:"rgba(79,158,122,0.1)",marginBottom:12 }}/>
          {[["📅",formatDate(date)],["🕐",slot.label],["⏱","1 hour"],["💰","200 000 DZD"]].map(([icon,val])=>(
            <div key={val} style={{ display:"flex",gap:10,fontSize:13,color:"#2a4a3e",marginBottom:6,fontWeight:500 }}>
              <span>{icon}</span><span>{val}</span>
            </div>
          ))}
        </div>
        <div style={{ fontSize:12.5,color:"#5a7a6e",marginBottom:22,lineHeight:1.65 }}>
          A confirmation email will be sent shortly. Your specialist will contact you before the session.
        </div>
...
// ── CIB Form ──────────────────────────────────────────────────────────────────
function CIBForm({ onPay, loading }) {
  ...
  if (step===2) return (
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      <div style={{ background:"#e8f5e9",borderRadius:14,padding:"14px 16px",marginBottom:20,display:"flex",alignItems:"center",gap:10 }}>
        ...
        <div style={{ fontSize:13,color:"#2d6b50",fontWeight:500 }}>An OTP has been sent to your registered phone number ending in <strong>****67</strong></div>
      </div>
      ...
      <button className="cp-pay-btn" onClick={()=>onPay()} disabled={otp.join("").length<6||loading}
        ...
        {loading?<><span style={{width:18,height:18,border:"2.5px solid rgba(245,230,66,0.3)",borderTopColor:"#f5e642",borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}/>Processing…</>:"Confirm Payment — 200 000 DZD"}
      </button>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      ...
      <div style={{background:"#f0f9f4",borderRadius:12,padding:"10px 13px",fontSize:12.5,color:"#2d6b50",display:"flex",alignItems:"center",gap:8,border:"1px solid rgba(45,122,79,0.15)"}}>
        ...
        An OTP will be sent to your registered phone to confirm the payment.
      </div>
...
// ── BaridiMob Form ────────────────────────────────────────────────────────────
function BaridiMobForm({ onPay, loading }) {
  ...
  if (step===2) return (
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      <div style={{background:"#fff8e1",borderRadius:14,padding:"14px 16px",marginBottom:20,border:"1px solid rgba(245,166,35,0.3)"}}>
        ...
        <div style={{fontSize:13,color:"#5a7a6e",lineHeight:1.6}}>Open your <strong>BaridiMob app</strong> and enter your <strong>5-digit PIN</strong> to confirm the payment of <strong>200 000 DZD</strong>.</div>
      </div>
      ...
      <button className="cp-pay-btn" onClick={()=>onPay()} disabled={pin.join("").length<5||loading}
        ...
        {loading?<><span style={{width:18,height:18,border:"2.5px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}/>Processing…</>:"Confirm with PIN — 200 000 DZD"}
      </button>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      <div style={{background:"#fff8e1",borderRadius:14,padding:"14px 16px",border:"1px solid rgba(245,166,35,0.3)",display:"flex",gap:12,alignItems:"flex-start"}}>
        ...
        <div style={{fontSize:13,color:"#5a7a6e",lineHeight:1.65}}>
          Enter your <strong style={{color:"#1a3329"}}>RIP (Relevé d'Identité Postal)</strong> — the 20-digit account number found in your BaridiMob app or Poste Algérie passbook.
        </div>
      </div>
      <div>
        <label className="cp-label">RIP — Compte CCP (20 chiffres)</label>
        <input className={`cp-input${errors.rip?" err":""}`} placeholder="00000 00000 00000 00000" value={rip} onChange={handleRipChange}
      ...
      <div style={{background:"#f0f9f4",borderRadius:12,padding:"10px 13px",fontSize:12.5,color:"#2d6b50",display:"flex",alignItems:"center",gap:8,border:"1px solid rgba(45,122,79,0.15)"}}>
        ...
        You will confirm with your 5-digit BaridiMob PIN in the next step.
      </div>
...
function IntlCardForm({ onPay, loading }) {
    ...
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <div>
          <label className="cp-label">CVV / CVC</label>
          <div style={{position:"relative"}}>
            <input className={`cp-input${errors.cvv?" err":""}`} placeholder="000" value={form.cvv} onChange={handle("cvv")} type="password" style={{paddingRight:40}}/>
            <span title="3 digits on the back of your card" style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",cursor:"help",color:"#9aac8a",fontSize:14}}>?</span>
          </div>
          <ErrMsg msg={errors.cvv}/>
        </div>
      </div>
      ...
      <div style={{background:"#f0f7ff",borderRadius:12,padding:"10px 13px",fontSize:12.5,color:"#1a6fa0",display:"flex",alignItems:"center",gap:8,border:"1px solid rgba(26,111,160,0.15)"}}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
        Payment equivalent: ~<strong style={{marginLeft:4}}>≈ 200 000 DZD</strong> · Secured by 3D Secure
      </div>
      <button className="cp-pay-btn" onClick={()=>{ if(validate()) onPay(); }} disabled={loading}
        style={{background:"linear-gradient(135deg,#1a3329,#2d6b50)",color:"#f5e642",boxShadow:"0 6px 20px rgba(26,51,41,0.28)"}}>
        {loading?<><span style={{width:18,height:18,border:"2.5px solid rgba(245,230,66,0.3)",borderTopColor:"#f5e642",borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}/>Processing…</>:"Pay — 200 000 DZD"}
      </button>
    </div>
  );
}
    ...
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      ...
      <div style={{background:"#003087",borderRadius:14,padding:"16px 18px",display:"flex",alignItems:"center",gap:12}}>
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
          <path fill="#009cde" d="M20.07 7.16C20.29 5.38 20.07 4.2 19.2 3.22 18.24 2.14 16.5 1.67 14.28 1.67H7.86a.96.96 0 0 0-.95.81L4.54 17.93a.58.58 0 0 0 .57.67h4.14l1.04-6.6-.03.2a.96.96 0 0 1 .95-.82h1.98c3.87 0 6.9-1.57 7.78-6.12.03-.13.05-.26.07-.38l.03-.08v.36z"/>
          <path fill="#012169" d="M9.58 8.22a.84.84 0 0 1 .83-.71h5.28c.63 0 1.21.04 1.74.13.15.02.3.05.44.08l.4.1c.13.04.26.08.38.13.57.22 1.06.53 1.44.95.24-1.5.01-2.53-.82-3.46C18.24 4.4 16.5 3.93 14.28 3.93H7.86a.96.96 0 0 0-.95.81L4.54 20.2a.58.58 0 0 0 .57.67h4.17l1.04-6.6 1.26-6.05z"/>
        </svg>
        <div>
      ...
      <div>
        ...
        <div style={{position:"relative"}}>
          ...
          <button type="button" onClick={()=>setShowPw(!showPw)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#9aac8a",display:"flex",lineHeight:0,padding:4}}>
            {showPw
      ...
      <div style={{background:"#f0f7ff",borderRadius:12,padding:"10px 13px",fontSize:12.5,color:"#1a6fa0",display:"flex",alignItems:"center",gap:8,border:"1px solid rgba(26,111,160,0.15)"}}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        Amount: ~<strong style={{marginLeft:4}}>≈ 200 000 DZD</strong> · Converted at current exchange rate
      </div>

      <button className="cp-pay-btn" onClick={()=>{ if(validate()) onPay(); }} disabled={loading}
        style={{background:"#003087",color:"#fff",boxShadow:"0 6px 20px rgba(0,48,135,0.35)"}}>
        {loading?<><span style={{width:18,height:18,border:"2.5px solid rgba(255,255,255,0.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}/>Processing…</>:"Pay with PayPal — 200 000 DZD"}
      </button>
    </div>
  );
}
...
export default function ConsultationPayment() {
  ...
  const { sp, date, slot } = location.state || {};
  ...
  if (!sp||!date||!slot) return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontSize:48,marginBottom:16}}>⚠️</div>
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:800,color:"#1a3329",marginBottom:12}}>No booking found</div>
        <button onClick={()=>navigate("/specialists")} style={{background:"linear-gradient(135deg,#1a3329,#2d6b50)",color:"#f5e642",border:"none",borderRadius:12,padding:"12px 28px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>← Back to Specialists</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f2f7f5",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"44px 24px 80px"}}>
        ...
        <button onClick={()=>navigate("/specialists")} style={{display:"inline-flex",alignItems:"center",gap:6,background:"none",border:"none",cursor:"pointer",fontSize:13.5,fontWeight:600,color:"#5a7a6e",fontFamily:"'DM Sans',sans-serif",marginBottom:28,padding:0}}>
        ...
        <div className="cp-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:26}}>
          ...
          <div className="cp-fade" style={{display:"flex",flexDirection:"column",gap:18}}>
            ...
            <div style={{background:"linear-gradient(135deg,#1a3329,#2d6b50)",borderRadius:22,padding:"24px",border:"1px solid rgba(245,230,66,0.12)"}}>
              <div style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.45)",letterSpacing:1,textTransform:"uppercase",marginBottom:12}}>Your Consultation</div>
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:18}}>
                <img src={sp.avatar} alt={sp.name} style={{width:54,height:54,borderRadius:"50%",objectFit:"cover",border:"2px solid rgba(255,255,255,0.25)",flexShrink:0}}/>
                <div>
                  <div style={{fontFamily:"'Syne',sans-serif",fontSize:17,fontWeight:800,color:"#fff"}}>{sp.name}</div>
                  <div style={{fontSize:12.5,color:"rgba(255,255,255,0.65)",marginTop:2}}>{sp.title}</div>
                </div>
              </div>
              <div style={{height:1,background:"rgba(255,255,255,0.1)",marginBottom:16}}/>
              {[["📅 Date",formatDate(date)],["🕐 Time",slot.label],["⏱ Duration","1 hour"],["📍 Format","Online video session"]].map(([k,v])=>(
                <div key={k} style={{display:"flex",justifyContent:"space-between",fontSize:13.5,marginBottom:10,gap:12}}>
            ...
            <div style={{background:"linear-gradient(135deg,#fefde8,#fdf6c0)",borderRadius:18,padding:"18px 20px",border:"1.5px solid rgba(245,230,66,0.4)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                ...
                <div style={{fontSize:12.5,color:"#5a7a6e",marginTop:2}}>One-time consultation fee</div>
              </div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:800,color:"#b8a200"}}>200 000 DZD</div>
            </div>

            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {[["🔒","256-bit SSL encrypted payment"],["📋","Instant booking confirmation"],["🔄","Free rescheduling 24h before"],["📞","Specialist contacts you before session"]].map(([icon,text])=>(
          ...
          <div className="cp-fade" style={{animationDelay:"0.12s"}}>
            <div style={{background:"#fff",borderRadius:22,padding:"26px 24px",boxShadow:"0 4px 24px rgba(26,51,41,0.08)",border:"1px solid rgba(79,158,122,0.1)"}}>
              ...
              {/* Payment form */}
              {method==="cib"       && <CIBForm       onPay={handlePay} loading={loading}/>}
              {method==="baridimob" && <BaridiMobForm  onPay={handlePay} loading={loading}/>}
              {method==="card"      && <IntlCardForm   onPay={handlePay} loading={loading}/>}
      ...
      {success&&<SuccessModal sp={sp} date={date} slot={slot} onClose={()=>{setSuccess(false);navigate("/profile");}}/>}
      <Footer/>
    </div>
  );
}
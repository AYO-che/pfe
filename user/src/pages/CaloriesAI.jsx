...
export default function CaloriesAI() {
    ...
    // ── QR session state (new, minimal) ──
    const [sessionId] = useState(getOrCreateSessionId);
    const [qrMode, setQrMode] = useState(false); // show QR overlay on desktop
    ...
    useEffect(() => {
        ...
        const unsub = onValue(dbRef.current, (snap) => {
            const data = snap.val();
            if (data?.photo) {
                // Photo arrived from phone — feed it into the existing flow
    ...
    const reset = () => {
        setStage("idle"); setImgSrc(null); setResult(null); setProgress(0); setQrMode(false);
    };
    ...
    return (
        <div style={{ minHeight: "100vh", background: "#f7faf8", fontFamily: "'DM Sans',sans-serif" }}>
            ...
            <section
                style={{
                    background:
                        "linear-gradient(135deg,#1a3329 0%,#2d6b50 60%,#1a3329 100%)",
                    padding: mob ? "20px 16px 20px" : "28px 32px 28px", // ✅ أصغر بكثير
                    position: "relative",
                    overflow: "hidden",
                }}
            >
                <div
                    style={{
                        ...
                        inset: 0,
                        backgroundImage:
                            "radial-gradient(circle,rgba(245,230,66,0.06) 1.5px,transparent 1.5px)",
                ...
                >
                    ...
                    >
                        Upload a photo of any meal. Our AI analyses the image and
                        returns the calorie count with your personalised daily intake
                        progress — in under 3 seconds.
                    </p>
                </div>
            </section>
            ...
            <div style={{ maxWidth: 1100, margin: "0 auto", padding: mob ? "40px 20px 80px" : "56px 40px 100px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : stage === "result" ? "1fr 1fr" : "1fr", gap: 28, marginBottom: stage === "idle" ? 72 : 0 }}>
                    ...
                    {stage === "idle" && !qrMode && (
                        <div className="cal-fade-up cal-upload-zone"
                            ...
                            <div style={{ display: "flex", justifyContent: "center", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
                                ...
                                {/* ── QR button (only on non-mobile) ── */}
                                {!mob && (
                                    <div
                                        onClick={e => { e.stopPropagation(); setQrMode(true); }}
                                        style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,0.9)", color: "#1a3329", border: "2px solid rgba(79,158,122,0.3)", borderRadius: 999, padding: "13px 32px", fontSize: 14.5, fontWeight: 700, letterSpacing: 0.3, cursor: "pointer" }}>
                    ...
                    {/* ── QR MODE ── */}
                    {stage === "idle" && qrMode && (
                        <div className="cal-fade-up" style={{ border: "2px dashed rgba(79,158,122,0.3)", borderRadius: 28, padding: mob ? "52px 24px" : "72px 40px", textAlign: "center", background: "rgba(255,255,255,0.85)", backdropFilter: "blur(8px)", boxShadow: "0 4px 24px rgba(26,51,41,0.07)" }}>
                            <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 24, fontWeight: 800, color: "#1a3329", marginBottom: 10 }}>Scan with your phone</div>
                            <div style={{ fontSize: 14.5, color: "#5a7a6e", marginBottom: 32, lineHeight: 1.7 }}>Open the camera on your phone and scan this code.<br />The photo will appear here automatically.</div>

                            <div style={{ display: "inline-block", padding: 20, background: "#fff", borderRadius: 20, border: "2px solid rgba(79,158,122,0.2)", boxShadow: "0 8px 32px rgba(26,51,41,0.1)", marginBottom: 32 }}>
                                <QRCodeSVG value={scanURL} size={200} />
                            </div>

                            <div style={{ display: "flex", alignItems: "center", gap: 10, justifyContent: "center", fontSize: 13, color: "#5a7a6e", marginBottom: 32 }}>
                                <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#2d9e7a", display: "inline-block", animation: "pulse 2s ease infinite" }} />
                                Waiting for photo from your phone…
                            </div>

                            <button
                                onClick={reset}
                                style={{ background: "transparent", color: "#5a7a6e", border: "1.5px solid rgba(79,158,122,0.25)", borderRadius: 999, padding: "10px 28px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
                                ← Back
                            </button>
                        </div>
                    )}
